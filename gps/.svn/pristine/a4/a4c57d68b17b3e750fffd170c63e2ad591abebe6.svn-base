...
<script type="text/javascript">
	...
	function clearForm () {
		...
		$("#secondCode").combobox("loadData",[{text:'',value:''}]); //防止切换类别的时候，缓存数据未清除 
	}
	...
	function loadCompany (id,param) {
		var url = "getVehicleInfo.do?parentCode="+param;
		$("#"+id).combobox({
			panelHeight:"150",
			url:url,
			valueField: 'id',    
        		...
        		$("#topCode").next('.combo').find('input').blur(function(){
        			...
        			for(var i = 0 ;i < data.length ;i++){
        				...
        				if(tmp.name == inputData){
        					...
        					loadOperation("secondCode",$("#topCode").combobox("getValue"));
        					return;
        				}
        			}
	...
	function loadOperation (id,param) {
		...
		$("#"+id).combobox({
        	...
        	onLoadSuccess : function (data){
        		//失去焦点触发事件，手动对combobox赋value值
        		$("#secondCode").next('.combo').find('input').blur(function(){
        			var inputData = $("#secondCode").combobox("getText");
        			var data = $("#secondCode").combobox("getData");
        			//console.log(inputData);
        			for(var i = 0 ;i < data.length ;i++){
        				var tmp = data[i];
        				if(tmp.name == inputData){
        					$("#secondCode").combobox("setValue",tmp.id);
        					//console.log("=========="+tmp.name);
        					return;
        				}
        			}
        			$("#secondCode").combobox("clear");
        		});
        	},
        	onSelect:function(record){
        		//比loadSuccess后触发，事件不触发手动获取value
        		//事件触发自动获取最终值
				//console.log("opree+++======"+$("#secondCode").combobox("getValue")+$("#"+id).combobox("getText"));
        	}
		});
	}
</script>
...
<body>
		<div class="easyui-panel" data-options="title:'车辆层次信息添加',collapsible:true">
		<form action="#" id="addForm" name="addForm">
			<table style="width:100%;" cellpadding="5" cellspacing="0" border="1">
				...
				<tr id="companyDiv2">
					...
					<td><input type="text" id="topCode" class="easyui-combobox" style="width:150px;" prompt="请选择公司名称" data-options="required:true">
						<span style="color:red">公司不存在时请先在维护类别添加</span></td>
				</tr>
				<!-- 运营属性区域 -->
				<tr id="oppDiv">
				...
				<tr id="oppDiv2">
					<td align="right" width="150px;"><span><font color="red">*</font></span>运营属性: </td>
					<td><input type="text" id="secondCode" class="easyui-combobox" style="width:150px;" prompt="请选择运营属性" data-options="required:true">
						<span style="color:red">运营属性不存在时请先在维护类别添加</span></td>
				</tr>
				<!-- 车辆类别 -->
				<tr id="vehicleDiv">
	...
	<div id="saveArea" name="saveArea" class="easyui-panel" data-options="title:'车辆层次待保存数据',collapsible:true">
		<div id="tempData">
			<table id="tempDatagrid" class="easyui-datagrid" width="100%" data-options="singleSelect:false">
				<thead>
					   <tr>
					        <th data-options="field:'ck',checkbox:true"></th>
					       	<th align="center" data-options="field:'topName',width:$$.fillsize(0.3)">公司名称</th>
					       	<th align="center" data-options="field:'secondName',width:$$.fillsize(0.3)">运营属性</th>
