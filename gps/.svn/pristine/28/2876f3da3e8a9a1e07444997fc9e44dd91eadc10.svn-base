...
import com.yunda.gps.monitor.service.ShipmentService;
...
public class ShipmentController extends BaseController{
	...
	@Resource(name="shipmentServiceImpl")
	...
	public void pageQuery(HttpServletRequest request,HttpServletResponse response,ShipmentPo shipment){
		Page page = shipmentService.pageQuery(shipment);
		if(page != null){
	...
	public void export(HttpServletResponse response, HttpServletRequest request){
		...
		String fileName = PropertiesManager.getProperty("export.shipment.fileName").replaceAll(".xls", DateUtil.format(new Date(), DateUtil.YYYYMMDD)+".xls");
		String gridName = PropertiesManager.getProperty("export.shipment.columnNames");
		String fieldName = PropertiesManager.getProperty("export.shipment.fieldNames");
		...
		ShipmentPo vo = (ShipmentPo) JSONObject.toBean(jsonObject, ShipmentPo.class);
		...
		try {
			...
			for(int i = 1 ; i <= count ;i++){
				...
				for(ShipmentPo value : list){
					...
					for(String fieldele : fieldName.split(",")){
						...
						Field field = ShipmentPo.class.getDeclaredField(fieldele);
			...
			JCDFWebUtil.Log(request, "导出货运单信息记录信息报表");
			if(out != null){
				try {
					out.close();
				} catch (IOException e) {
