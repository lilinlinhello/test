...
public class BrandController extends BaseController {
	...
	public String forwardListPage(HttpServletRequest request){
		...
		return "equipmentBrand/equipmentBrand_list";
	}
	
	@RequestMapping("/addPage.do")
	public String forwardAddPage(){
		return "equipmentBrand/equipmentBrand_add";
	}
	
	@RequestMapping("/editPage.do")
	public String forwardEditPage(HttpServletRequest request,HttpServletResponse response){
		return "equipmentBrand/equipmentBrand_edit";
	}
	
	@RequestMapping("/getBrandById.do")
	public void getBrandById(HttpServletResponse response,String type,String childCode){
		VehicleCategoryVo vehicle = new VehicleCategoryVo();
		VehicleCategoryVo result = null;
		if(StringUtil.isNotNullStr(type) && type.equals(Constant.TYPE_00)){//编辑设备
		...
		if(StringUtil.isNotNullStr(type) &&type.equals(Constant.TYPE_01)){//编辑型号
			result = vehicleCategoryService.selectById(childCode);
		...
		if(StringUtil.isNotNullStr(type) && type.equals(Constant.TYPE_02)){//编辑
			result = vehicleCategoryService.selectById(childCode);
	...
	public void updateBrand(HttpServletRequest request,HttpServletResponse response,VehicleCategoryVo vo){
		...
		if(StringUtil.isNotNullStr(type) && type.equals(Constant.TYPE_00)){//编辑设备品牌
			vo.setParentCode("0");
		...
		if (null != msg && msg.isResult()) {
			JCDFWebUtil.Log(request, "修改品牌分类信息：childCode="+vo.getChildCode()+",分类名称="+vo.getName());
		}
	 ...
	 */
	@RequestMapping("/getBrandInfo.do")
	public void getBrandInfo(HttpServletRequest request,HttpServletResponse response,VehicleCategoryVo vo){
		vo.setModelType("brand");
	 ...
	 * @param response
	 */
	@RequestMapping("/insertBrands.do")
	public synchronized void insertBrands(HttpServletRequest request,HttpServletResponse response){
	...
	public void export(HttpServletResponse response, HttpServletRequest request){
		...
		String fileName = PropertiesManager.getProperty("export.brand.fileName").replaceAll(".xls", DateUtil.format(new Date(), DateUtil.YYYYMMDD)+".xls");
		String gridName = PropertiesManager.getProperty("export.brand.columnNames");
		String fieldName = PropertiesManager.getProperty("export.brand.fieldNames");
		...
		try {
			...
			JCDFWebUtil.Log(request, "导出设备品牌信息报表");
			if(out != null){
				try {
					out.close();
				} catch (IOException e) {
