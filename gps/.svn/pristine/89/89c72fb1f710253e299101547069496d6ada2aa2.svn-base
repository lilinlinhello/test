...
@Controller
@RequestMapping(value="/shipmentMonitor")
public class ShipmentMonitorController extends BaseController{

	@Resource(name="shipmentMonitorServiceImpl")
	private ShipmentMonitorService shipMonitorService;
	...
	public String forwardListPage(HttpServletRequest request){
		return "monitor/shipmentMonitor_list";
	}
	...
	public void pageQuery(HttpServletRequest request,HttpServletResponse response,ShipmentMonitorVo shipment){
		Page page = shipMonitorService.pageQuery(shipment);
		if(page != null){
	...
	public void export(HttpServletResponse response, HttpServletRequest request){
		...
		String fileName = PropertiesManager.getProperty("export.shipmentMonitor.fileName").replaceAll(".xls", DateUtil.format(new Date(), DateUtil.YYYYMMDD)+".xls");
		String gridName = PropertiesManager.getProperty("export.shipmentMonitor.columnNames");
		String fieldName = PropertiesManager.getProperty("export.shipmentMonitor.fieldNames");
		...
		ShipmentMonitorVo vo = (ShipmentMonitorVo) JSONObject.toBean(jsonObject, ShipmentMonitorVo.class);
		...
		try {
			...
			for(int i = 1 ; i <= count ;i++){
				...
				for(ShipmentMonitorPo value : list){
					...
					for(String fieldele : fieldName.split(",")){
						...
						Field field = ShipmentMonitorPo.class.getDeclaredField(fieldele);
			...
			JCDFWebUtil.Log(request, "导出货运单监控记录信息报表");
			if(out != null){
				try {
					out.close();
				} catch (IOException e) {
