...
public final class JedisOpExecuter {
	 ...
	 * @throws JedisException
	 */
	public static void putSingleObject(Object key, Object value,Integer seconds)  throws JedisException{
		try{
			if(key != null){
				JedisPool jedisPool = JedisObjectFactory.getJedisPoolInstance();
				Jedis redisClient = jedisPool.getResource();
				JedisOpExecuter.putObject(jedisPool, redisClient, key, value , seconds);
			}
		}catch(Exception e){
			//logger.debug("putObject:"+key+"="+value+" &failed Error msg:  "+e.getMessage());
			throw new JedisException(e.getMessage());
	 ...
	 * @param seconds
	 */
	private static void putObject(JedisPool jedisPool, Jedis redisClient, Object key, Object value,Integer seconds) {
		try {
			redisClient.setex(SerializeUtil.serialize(key.toString()), seconds, SerializeUtil.serialize(value));
		} catch (Exception e) {
			JedisOpExecuter.returnBrokenJedisInstance(jedisPool, redisClient);
		} finally {
			JedisOpExecuter.returnJedisInstance(jedisPool, redisClient);
