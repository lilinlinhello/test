...
import com.yunda.gps.monitor.service.AlarmLoggingService;
import com.yunda.gps.util.DateUtil;
import com.yunda.gps.util.PropertiesManager;
import com.yunda.gps.util.StringUtil;

@Controller
@RequestMapping(value="/alarmLogging")
public class AlarmLoggingController extends BaseController{

	
	@Resource(name="alarmLoggingServiceImpl")
	...
	public String forwardListPage(HttpServletRequest request){
		...
		//字典类型
		//params.put("alarmLogging","dic_alarmLogging");
		...
		return "monitor/alarmLogging_list";
	}
	...
	public void pageQuery(HttpServletRequest request,HttpServletResponse response,AlarmLogging alarmLogging){
		Page page = alarmLoggingService.pageQuery(alarmLogging);
		if(page != null){
	...
	public void export(HttpServletResponse response, HttpServletRequest request){
		...
		String fileName = PropertiesManager.getProperty("export.alarmLogging.fileName").replaceAll(".xls", DateUtil.format(new Date(), DateUtil.YYYYMMDD)+".xls");
		String gridName = PropertiesManager.getProperty("export.alarmLogging.columnNames");
		String fieldName = PropertiesManager.getProperty("export.alarmLogging.fieldNames");
		...
		AlarmLogging vo = (AlarmLogging) JSONObject.toBean(jsonObject, AlarmLogging.class);
		...
		try {
			...
			for(int i = 1 ; i <= count ;i++){
				...
				for(AlarmLogging value : list){
					...
					for(String fieldele : fieldName.split(",")){
						...
						Field field = AlarmLogging.class.getDeclaredField(fieldele);
			...
			JCDFWebUtil.Log(request, "导出报警记录信息报表");
			if(out != null){
				try {
					out.close();
				} catch (IOException e) {
