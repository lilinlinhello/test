...
import com.yunda.gps.monitor.service.LineMileageService;
import com.yunda.gps.util.DateUtil;
import com.yunda.gps.util.PropertiesManager;
import com.yunda.gps.util.StringUtil;

@Controller
@RequestMapping(value="/lineMileage")
public class LineMileageController extends BaseController{
	
	@Resource(name="lineMileageServiceImpl")
	private LineMileageService lineMileageService;
...
//		params.put("lineMileage","dic_lineMileage");
//		params.put("txdFlag","dic_txdFlag");
...
//		request.setAttribute("dictionarys", JSONArray.fromObject(dics));
		return "monitor/lineMileage_list";
	}
	...
	public void pageQuery(HttpServletRequest request,HttpServletResponse response,LineMileage lineMileage){
		Page page = lineMileageService.pageQuery(lineMileage);
		if(page != null){
	...
	public void export(HttpServletResponse response, HttpServletRequest request){
		...
		String fileName = PropertiesManager.getProperty("export.lineMileage.fileName").replaceAll(".xls", DateUtil.format(new Date(), DateUtil.YYYYMMDD)+".xls");
		String gridName = PropertiesManager.getProperty("export.lineMileage.columnNames");
		String fieldName = PropertiesManager.getProperty("export.lineMileage.fieldNames");
		...
		LineMileage vo = (LineMileage) JSONObject.toBean(jsonObject, LineMileage.class);
		...
		try {
			...
			for(int i = 1 ; i <= count ;i++){
				...
				for(LineMileage value : list){
					...
					for(String fieldele : fieldName.split(",")){
						...
						Field field = LineMileage.class.getDeclaredField(fieldele);
			...
			JCDFWebUtil.Log(request, "导出线路里程信息报表");
			if(out != null){
				try {
					out.close();
				} catch (IOException e) {
