...
import com.yunda.gps.monitor.service.ShipmentMonitorErrorService;
import com.yunda.gps.util.DateUtil;
import com.yunda.gps.util.PropertiesManager;
import com.yunda.gps.util.StringUtil;

@Controller
@RequestMapping(value="/shipmentMonitorError")
public class ShipmentMonitorErrorController extends BaseController {

	@Resource(name="shipmentMonitorErrorServiceImpl")
	private ShipmentMonitorErrorService shipMonitorErrorService;
	...
	public String forwardListPage(HttpServletRequest request){
		return "monitor/shipmentMonitorError_list";
	}
	...
	public void pageQuery(HttpServletRequest request,HttpServletResponse response,ShipmentMonitorError ship){
		Page page = shipMonitorErrorService.pageQuery(ship);
		if(page != null){
	...
	public void export(HttpServletResponse response, HttpServletRequest request){
		...
		String fileName = PropertiesManager.getProperty("export.shipmentMonitorError.fileName").replaceAll(".xls", DateUtil.format(new Date(), DateUtil.YYYYMMDD)+".xls");
		String gridName = PropertiesManager.getProperty("export.shipmentMonitorError.columnNames");
		String fieldName = PropertiesManager.getProperty("export.shipmentMonitorError.fieldNames");
		...
		ShipmentMonitorError vo = (ShipmentMonitorError) JSONObject.toBean(jsonObject, ShipmentMonitorError.class);
		...
		try {
			...
			for(int i = 1 ; i <= count ;i++){
				...
				for(ShipmentMonitorError value : list){
					...
					for(String fieldele : fieldName.split(",")){
						...
						Field field = ShipmentMonitorError.class.getDeclaredField(fieldele);
			...
			JCDFWebUtil.Log(request, "导出货运单监控异常记录信息报表");
			if(out != null){
				try {
					out.close();
				} catch (IOException e) {
